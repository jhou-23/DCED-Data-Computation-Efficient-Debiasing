...
def train(args, data, datasets, model: PreTrainedModel, original_model, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    def attribute_vector_example():
        ...
        for key in distribution:
            if key != 'neutral':
                inputs, labels = next(dataloaders[key])     # guessing it is the position of the token
                inputs = inputs.to(args.device)     # 32xvariable length
        ...
        return attributes_hiddens       # attributes_hiddens["attribute0"]--> 1 x 13 x 768 is stack
    ...
    def forward(attributes_hiddens, dataloaders, key):
        ...
        if args.model_type == 'bert':
            ...
            if 'neutral' != key:
                temp_og = original_model.bert(inputs)
                with torch.no_grad():
                    final_layer_original_hiddens, _, all_layer_original_hiddens = temp_og.last_hidden_state, temp_og.pooler_output, temp_og.hidden_states
                if args.token_loss:
        ...
        if args.loss_target == 'sentence' or labels is None:
            ...
            if 'neutral' != key:
                ...
                attributes_hiddens = {key: value.expand(target_layer_hiddens.size(0),
                                                        1,
                                                        value.size(1),  # num words
                                                        value.size(2))
                                      for key, value in attributes_hiddens.items()}
        if 'neutral' == key:
            loss = 0
            tmp_loss = criterion_ip(target_layer_hiddens, attributes_hiddens["attribute0"] - attributes_hiddens["attribute1"])
            if args.square_loss:
                tmp_loss = tmp_loss ** 2
            tmp_loss *= alpha
            loss += tmp_loss
...
def main():
    ...
    if args.adapter:
        ...
        model.add_adapter('equal_loss', config=adapter_config)
        model.train_adapter('equal_loss')
        model.set_active_adapters('equal_loss')
