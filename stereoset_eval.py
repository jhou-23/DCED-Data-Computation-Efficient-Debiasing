...
import stereoset_data as dataloader
...
if __name__ == "__main__":
    ...
    if args.predictions_dir is not None:
        ...
        for prediction_file in glob.glob(predictions_dir + "*.json"):
            split = _extract_split_from_file_path(prediction_file)

            print()
            print(f"Evaluating {prediction_file}...")
            # We use the corresponding gold file for each prediction file.
            parse_file(
                f"{args.persistent_dir}/data/stereoset/{split}.json", prediction_file
            )
    else:
        split = _extract_split_from_file_path(args.prediction_file)
        parse_file(
            f"{args.persistent_dir}/data/stereoset/{split}.json", args.predictions_file
        )