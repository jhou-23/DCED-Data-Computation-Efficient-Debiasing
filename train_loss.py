...
def main():
    ...
    if args.dataset_name is not None:
        ...
        if "validation" not in raw_datasets.keys():
            ...
            raw_datasets["train"] = raw_datasets["train"].filter(lambda example: onein(example))
    else:
        data_files = {}
        if args.train_file is not None:
    ...
    def replace_tgt(examples):
        ...
        text = ""
        for word in words:
            if word.strip() in tgt_map.keys():
                if word.strip() == word:
                    text +=  tgt_map[word] + " "
                else:
                    text += tgt_map[word.strip()] + "\n "
            else:
                text += word + " "
    ...
    with accelerator.main_process_first():
        raw_datasets = raw_datasets.map(replace_tgt)
